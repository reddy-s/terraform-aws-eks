...
resource "aws_cloudformation_stack" "workers_launch_template" {
  ...
  tags = "${merge(var.tags, map("key", "Name", "value", "${aws_eks_cluster.this.name}-${lookup(var.worker_groups_launch_template[count.index], "name", count.index)}-eks_asg", "propagate_at_launch", true))}"
