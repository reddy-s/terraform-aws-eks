...
|------|-------------|:----:|:-----:|:-----:|
| allowed\_cidr\_blocks\_cluster | List of CIDR blocks to be allowed to connect to the EKS cluster | list | `[]` | no |
| allowed\_cidr\_blocks\_workers | List of CIDR blocks to be allowed to connect to the worker groups | list | `[]` | no |
| allowed\_security\_groups\_cluster | List of Security Group IDs to be allowed to connect to the EKS cluster | list | `[]` | no |
| allowed\_security\_groups\_workers | List of Security Group IDs to be allowed to connect to the worker groups | list | `[]` | no |
| autoscaling\_policies\_enabled | Setup autoscaling policies on worker groups. Can be used instead of cluster autoscaler, but should not be turned on if cluster autoscale is enabled | string | `"false"` | no |
| aws\_cli\_image |  | string | `"quay.io/coreos/awscli:master"` | no |
...
| node\_drain\_enabled | Whether node drain should be enabled on node group updates | string | `"false"` | no |
...
| kube\_node\_drainer\_asg\_ds | Kubernetes daemonset to perform node draining |
| kube\_node\_drainer\_asg\_status\_updater | Kubernetes deployment to update status when nodes are drained |
| kube\_rbac | Role and rolebinding to let node drainer work as needed. |
